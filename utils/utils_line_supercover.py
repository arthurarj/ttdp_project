...
### Private methods
    ...
    def _mapDataSampleToGrid(self, sample, assigning_map):
        ...
        covered_cells = self._getSuperCover(pu,do)
...
### Public methods
    ...
    def fitGrid(self, dataset, func):
        ...
        prior = getattr(dataset.trip_ratio, func)()
        self.grid_model = prior * np.ones((self.hsize, self.vsize))
        for (i,j), value in assigning_map.items():
            self.grid_model[i][j] = getattr(dataset.loc[assigning_map[(i,j)]].trip_ratio, func)()
        return self.grid_model

    def _predictFactor(self, sample):
        pu = np.array([sample.pu_lon, sample.pu_lat])
        do = np.array([sample.do_lon, sample.do_lat])
        cells = self._getSuperCover(pu, do)
        return np.mean([self.grid_model[i][j] for (i,j) in cells])
    
    def predictFactor(self, data):
        return data.apply(lambda s: self._predictFactor(s), axis=1)   